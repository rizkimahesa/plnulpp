            ...
            <thead class="bg-gradient-to-r from-blue-600 to-indigo-500 text-white uppercase text-xs sticky top-0 z-10">
                <tr>
                    @foreach ($data[0] as $header)
                        <th class="px-6 py-4 border border-gray-300 dark:border-gray-600 font-semibold whitespace-nowrap">
                            {{ $header }}
                        </th>
                    @endforeach
                </tr>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @foreach (array_slice($data, 1) as $index => $row)
                    <tr class="{{ $index % 2 === 0 ? 'bg-gray-50 dark:bg-gray-900' : '' }} hover:bg-indigo-50 dark:hover:bg-indigo-700 transition duration-150">
                        @foreach ($row as $cell)
                            <td class="px-6 py-4 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 break-words max-w-[250px]">
                                {{ $cell }}
                            </td>
                        @endforeach
                    </tr>
